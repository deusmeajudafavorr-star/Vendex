  ...
  return (
    <div className="relative w-full h-[100dvh] max-h-[100dvh] bg-black snap-start snap-always overflow-hidden flex items-center justify-center select-none">
      <video
        ...
        className={`w-full h-full object-cover cursor-pointer ${video.is_locked_priority ? 'blur-md brightness-50' : ''}`}
      />
      ...
      {video.is_locked_priority && (
        <div className="absolute inset-0 z-35 bg-black/80 backdrop-blur-xl flex flex-col items-center justify-center p-6 text-center animate-fade-in pointer-events-auto">
          ...
          <h3 className="text-xl font-black text-white leading-tight max-w-xs">🔥 VÍDEO FRESQUINHO</h3>
          ...
          <div className="my-4 p-3 rounded-2xl bg-zinc-900/90 border border-zinc-800 w-full max-w-xs text-left flex items-center gap-3 shadow-xl">
            {video.thumbnail_url && (
              <img src={video.thumbnail_url} alt="" className="w-12 h-12 rounded-xl object-cover shrink-0 border border-white/10" />
            )}
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-white truncate">{video.title}</p>
              <p className="text-xs text-emerald-400 font-extrabold mt-0.5">{video.price || 'Oferta Exclusiva'}</p>
            </div>
          </div>
          <button
      ...
      {!isPlaying && videoLoaded && isActive && (
        <div onClick={togglePlayPause} className="absolute inset-0 flex items-center justify-center bg-black/20 pointer-events-auto cursor-pointer">
      ...
      <div className="absolute right-3.5 bottom-28 z-20 flex flex-col items-center gap-4 text-white">
        <button onClick={handleLike} className="group flex flex-col items-center gap-1 focus:outline-none transition-transform active:scale-90" title="Curtir produto">
          <div className={`w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-md transition-colors ${liked ? 'bg-rose-600 text-white shadow-lg shadow-rose-600/40' : 'bg-zinc-900/60 hover:bg-zinc-800/80 text-white'}`}>
            <Heart className={`w-6 h-6 transition-all ${liked ? 'fill-current scale-110' : 'stroke-[2]'}`} />
          </div>
          <span className="text-[11px] font-semibold tracking-wide drop-shadow-md">{likeCount}</span>
        </button>

        <button onClick={() => onOpenFreshModal?.(video)} className="group flex flex-col items-center gap-1 focus:outline-none transition-transform active:scale-90" title="🔥 Vídeo Fresquinho - Liberar acesso antecipado">
          ...
          <span className="text-[10px] font-extrabold text-amber-300 tracking-tight drop-shadow-md text-center leading-tight">Fresquinho</span>
        </button>

        <button onClick={handleShare} className="group flex flex-col items-center gap-1 focus:outline-none transition-transform active:scale-90" title="Compartilhar produto">
          <div className="w-12 h-12 rounded-full bg-zinc-900/60 hover:bg-zinc-800/80 backdrop-blur-md flex items-center justify-center text-white">
            {shareSuccess ? <Check className="w-6 h-6 text-emerald-400" /> : <Share2 className="w-6 h-6 stroke-[2]" />}
          </div>
          <span className="text-[11px] font-semibold tracking-wide drop-shadow-md">{shareSuccess ? 'Copiado!' : 'Compartilhar'}</span>
        </button>

        <button onClick={onToggleMute} className="group flex flex-col items-center gap-1 focus:outline-none transition-transform active:scale-90" title={isMuted ? 'Ativar som' : 'Desativar som'}>
          <div className={`w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-md transition-colors ${!isMuted ? 'bg-amber-500/80 text-white' : 'bg-zinc-900/60 hover:bg-zinc-800/80 text-white'}`}>
            {isMuted ? <VolumeX className="w-6 h-6 stroke-[2]" /> : <Volume2 className="w-6 h-6 stroke-[2]" />}
          </div>
          <span className="text-[11px] font-semibold tracking-wide drop-shadow-md">{isMuted ? 'Mudo' : 'Som'}</span>
        </button>

        {video.allow_download && (
          <button onClick={handleDownload} className="group flex flex-col items-center gap-1 focus:outline-none transition-transform active:scale-90" title="Baixar vídeo MP4 do produto">
            ...
            <span className="text-[11px] font-black tracking-wide text-emerald-300 drop-shadow-md">BAIXAR</span>
          </button>
        )}
      </div>
      ...
      <div className="absolute left-0 right-16 bottom-6 z-20 px-4 flex flex-col gap-2.5 text-white">
        {video.priority_release && (
          <div className="flex items-center gap-1.5 flex-wrap animate-fade-in">
            ...
            <span className="text-[10px] font-bold text-amber-300 bg-black/60 px-2 py-0.5 rounded-full backdrop-blur-sm border border-amber-500/30">⚡ ACABOU DE CHEGAR</span>
          </div>
        )}
        ...
        {video.tags && video.tags.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5 max-w-full">
            {video.tags.slice(0, 3).map((tag, idx) => (
              <button key={idx} onClick={(e) => { e.stopPropagation(); onSelectTag?.(tag); }} className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md text-zinc-200 border border-white/10 flex items-center gap-1 transition-colors">
            ...
            {video.title && (
              <>
                <div className="flex items-start gap-2 flex-wrap">
                  <h2 className="text-base sm:text-lg font-bold line-clamp-2 leading-snug drop-shadow-lg text-white">{video.title}</h2>
                ...
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  <span className="px-2 py-0.5 rounded-full bg-amber-500/20 border border-amber-400/40 text-amber-200 text-[9px] font-black uppercase">🚀 Altíssima chance de bombar</span>
                  <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-400/30 text-emerald-200 text-[9px] font-bold">👀 Oferta que chama atenção</span>
                </div>
              </>
            )}
            {!video.priority_release && video.price && (
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-emerald-400 font-extrabold text-base tracking-tight drop-shadow">{video.price}</span>
                {video.discount && <span className="text-[11px] font-bold bg-rose-600/90 text-white px-1.5 py-0.5 rounded-md">{video.discount}</span>}
              </div>
            )}
          </div>
        )}

        <p className="text-xs text-zinc-300 line-clamp-3 leading-relaxed drop-shadow max-w-[95%]">{standardDescription}</p>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1 max-w-md">
          ...
          {video.allow_download && (
            <button onClick={handleDownload} className="group relative py-3 px-4 rounded-2xl bg-gradient-to-r from-emerald-400 via-green-500 to-emerald-700 hover:from-emerald-300 hover:via-green-400 hover:to-emerald-600 text-white font-black text-xs backdrop-blur-md border border-emerald-200/70 flex items-center justify-center gap-1.5 active:scale-95 transition-all duration-200 shadow-xl shadow-emerald-950/70 ring-2 ring-emerald-300/50 animate-pulse overflow-hidden">
              ...
              {!downloadStatus && <span className="relative ml-0.5 w-2 h-2 rounded-full bg-yellow-300 shadow-[0_0_8px_rgba(253,224,71,0.9)]" />}
            </button>
          )}
        </div>
